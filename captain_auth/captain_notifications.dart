...
class CaptainNotificationsScreen extends StatefulWidget {
  @override
  _CaptainNotificationsScreenState createState() => _CaptainNotificationsScreenState();
}

class _CaptainNotificationsScreenState extends State<CaptainNotificationsScreen> {
  late Future<List<QueryDocumentSnapshot>> _futureNotifications;
  late Future<List<QueryDocumentSnapshot>> _futureCaptainsNotifications;
  ...
  void initState() {
    ...
    _futureCaptainsNotifications = _fetchCaptainsNotifications();
  }
  ...
  Future<List<QueryDocumentSnapshot>> _fetchCaptainsNotifications() async {
    final captainsSnapshot = await FirebaseFirestore.instance
        .collection('notifications_to_captains')
        .orderBy('timestamp', descending: true)
        .get();

    return captainsSnapshot.docs;
  }
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          bottom: TabBar(
            ...
            tabs: [
              ...
              Tab(
                child: Text(
                  _currentGroupTag == null
                      ? 'Χωρίς Group'
                      : 'Group $_currentGroupTag',
                  style: TextStyle(color: Colors.white),
                ),
              ),
              Tab(
                child: Marquee(
                  text: 'Ειδοποιήσεις Αρχηγών',
                  style: TextStyle(color: Colors.white),
                  scrollAxis: Axis.horizontal,
                  blankSpace: 50.0,
                  velocity: 50.0,
        ...
        body: TabBarView(
          children: [
            ...
            _buildCaptainsNotificationsTab(),
          ],
        ),
      ),
    );
  ...
  Widget _buildCaptainsNotificationsTab() {
    return Container(
      color: Colors.purpleAccent,
      child: FutureBuilder<List<QueryDocumentSnapshot>>(
        future: _futureCaptainsNotifications,
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return Center(child: CircularProgressIndicator());
          }
          ...
          if (!snapshot.hasData || snapshot.data!.isEmpty) {
            return Center(
                child: Text('Δεν υπάρχουν ειδοποιήσεις για τους αρχηγούς.'));
          }
