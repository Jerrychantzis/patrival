...
class AddMember extends StatefulWidget {
  ...
  const AddMember({Key? key, required this.groupId, required this.isAdmin}) : super(key: key);
...
class _AddMemberState extends State<AddMember> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Προσθήκη Χρηστών'),
        backgroundColor: Colors.lightBlue[100],
      ),
      body: StreamBuilder<QuerySnapshot>(
        stream: usersCollection.where('group', isEqualTo: 0).snapshots(),
    ...
    try {
      ...
      for (String username in selectedUsers) {
        ...
        if (userSnapshot.exists) {
          ...
          tags.remove("0");
          tags.add(widget.groupId);

          batch.update(usersCollection.doc(username), {
            'group': int.parse(widget.groupId),
            'tags': tags,
          });
      ...
      batch.update(
        groupRef,
        {'member_count': currentMemberCount + selectedUserIds.length},
      );

      await batch.commit();

      showToastGood(message: 'Επιτυχής Προσθήκη Χρηστών');
